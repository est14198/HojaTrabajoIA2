...
def formar_matrices(flat_thetas, shapes):
    layers = len(shapes) + 1
...
def funcion_costo(flat_thetas, shapes, X, Y):
    a = feed_forward(
        formar_matrices(flat_thetas, shapes),
        X
    )
    return -(Y * np.log(a[-1]) + (1 - Y) * np.log(1 - a[-1])).sum() / len(X)

def jacobiano(flat_thetas, shapes, X, Y):
    m, layers = len(X), len(shapes) + 1
    thetas = formar_matrices(flat_thetas, shapes)
